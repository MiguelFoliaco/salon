...
export const Canceled = () => {
    ...
    if (schedules.length === 0 && !loading) {
        return (
            <div className="py-20 flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-2">No hay citas canceladas</h3>
                <p className="text-slate-500">No tienes ninguna cita cancelada todavía.</p>
    ...
    return (
        <section>
            <h2 className="text-lg font-bold mb-4 text-slate-400 mt-8">Citas Canceladas</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {schedules.map((exp) => (
                    <div key={exp.id} className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-slate-100 flex items-center justify-between opacity-75">
                        <div className="flex gap-4 items-center">
                            <div className="w-16 h-16 rounded-xl overflow-hidden relative shrink-0 shadow-sm border border-slate-100 grayscale">
